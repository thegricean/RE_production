...
Empirical utterance proportions are shown against MAP model  predictions in \figref{fig:nompredictives} ($r=.86$). While the model overpredicts subordinate level  and underpredicts basic level choices in the \emph{sub necessary} condition, it generally captures the patterns in the data. The correlation between empirical utterance proportions and the best model's MAP predictions at the by-item level was $r = .86$.
