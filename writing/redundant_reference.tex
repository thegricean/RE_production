...
\ndg{consider alternatives: "rationally redundant referring expressions", "A Bayesian approach to redundancy in reference", ...? }
}
...
\ndg{note to self: review after full read.}
...
\ndg{while this is amusing... it's not really very relevant?}
}
%\end{flushright}
...
How should a cooperative speaker choose between competing referring expressions? Grice, in his seminal work, provided some guidance by formulating his famous conversational maxims, intended as a guide to listeners' expectations about good speaker behavior \cite{grice1975}. His maxim of Quantity, consisting of two parts, requires of speakers to:
...
As has been pointed out by \citeA{GattEtAl2013}, the basic Rational Speech Act model described so far \cite{frank2012} does not generate overinformative referring expressions for two reasons. 
...
\ndg{i moved all this up from expt section, since it is fundamentally a demonstration of model predictions}
...
In Sections 3, 4, and 5 we quantitatively evaluate continuous semantics RSA on datasets capturing the phenomena described in the Introduction (\tableref{tab:effects}): modifier type and scene variation effects on modified referring expressions, typicality effects on color mention, and the choice of taxonomic level of reference in nominal choice.
...
We saw in \sectionref{sec:modelkoolen} that continuous semantics RSA correctly predicts qualitative effects of scene variation on redundant adjective use. In particular, we saw that color is more likely to be used redundantly when objects vary along more dimensions. 
...
\caption{Empirical utterance proportions  (red)  alongside point-wise maximum a posteriori (MAP) estimates of the RSA model's posterior predictives for utterance probability (blue) as a function of scene variation. Rows indicate the sufficient dimension, columns the produced utterance. \ndg{could we adjust the columns labels to something like "color sufficient"? otherwise the graph takes a lot of decoding...} Here and in all following plots, error bars indicate 95\% bootstrapped confidence intervals.}
...
\ndg{say something about whether color-sufficient conditions show a scene variation effect? (i don't think the above analysis actually says one way or another...)}
...
In particular, we could imagine a continuous semantics in which \emph{banana} fits better (i.e. has a semantic value closer to 1 for) the yellow banana than the brown, and fits the brown better than the blue; specific such values are shown in the first row of \tableref{tab:colorobjectfidelities}.
If we then use the same semantic values for color adjectives (close to deterministic) and the same composition rule (product), we get the utterance-level semantic value shown in the remainder of \tableref{tab:colorobjectfidelities}.
Notice that while the utterance \emph{banana} on its own has significant semantic gradience, the utterances \emph{yellow/brown/blue banana} have very little gradience---this is because the color adjectives are very close to noiseless. \ndg{rephrase to point out that RSA only cares up to normalization. perhaps normalize rows in the table?}

Using these contexts and semantic values in the RSA model, with $\beta_i$ = 12 and $\beta_c$ = 5 (that is, both informativeness and utterance cost receive a substantial weight), the resulting speaker probabilities for the (minimal) utterance \emph{banana} are \ndg{fixme} .99, .37, and .05, to refer to the yellow banana, the brown banana, and the blue banana, respectively. In contrast, the resulting speaker probabilities for the redundant \emph{yellow banana}, \emph{brown banana}, and \emph{blue banana} are .01, .63, and .95, respectively. That is, redundant color mention increases with decreasing semantic value of the simple \emph{banana} utterance.
This shows that the relaxed semantics RSA can predict typicality effects if the semantic fit of the noun (and hence also of color-noun compounds) to an object is modulated by typicality.

%Continuous semantics RSA as we have set it up thus far does not capture this asymmetry: it knows that a particular modifier is a color modifier with a particular semantic value; it does not know anything about the typicality of the denoted properties for the referent. 
%
...
Two additional, more theoretically fraught, simplifications concern where typicality can enter into the semantics and how compositions proceeds. In the above, we have assumed that typicality directly affects only noun interpretations, however, it is very plausible that color adjectives are also affected by typicality---for instance, a color nearby blue, like purple, may still be a better fit to \emph{blue} than a far one, like orange. More importantly, there may be complex interactions between colors and kinds, such that what is a typical blue for one category (e.g. the sky) is actually unusual for another (e.g. fruit). Indeed, compositional semantics of typicality is a well known problem for theories of modification \cite{kamp1995}. Rather than try to solve it here, we note that RSA works at the level of whole utterances. Hence, if we can reasonably measure the semantic fit of each utterance to each possible referent, then RSA will make predictions for production without the need to derive the semantic values compositionally. 
That is, if we can measure the typicality of the phrase \emph{blue banana} for a banana, we don't need to derive it from \emph{blue}, \emph{banana}, and a theory of composition.
This separates pragmatic aspects of reference, which are the topic of this paper, from issues in compositional semantics, which are not; hence we will take this approach for experimentally testing the predictions of relaxed semantics RSA for typicality effects.
...
Finally, there were 10 speakers who consistently used roundabout descriptions instead of direct referring expressions (e.g., \emph{monkeys love\dots} to refer to banana).\footnote{It is unclear whether these participants misunderstood task instructions, or were simply being playful.\ndg{jd, is this reasonable? do we know anything else from the exit questions?}} These pairs were also excluded, since such indirect expressions do not inform our questions about modifier production.
...
As an example, the means for the banana items are shown in \tableref{tab:bananatypicalities}. 
The values are very similar to those hypothesized for the purpose of the example in \tableref{tab:colorobjectfidelities}. \ndg{might need to say up to normalization of rows, depending on what we do in the earlier table?}
...
Here, $p(u)$ is utterance frequency \ndg{log frequency? or inverse frequency?} as estimated from the Google Books corpus (years 1950 to 2008); $l(u)$ is the mean empirical length of the utterance in characters in the production data (e.g., sometimes \emph{yellow} was abbreviated as \emph{yel}, leading to an $l(u)$ smaller than 6); $\beta_F$ is a weight on frequency; and $\beta_L$ is a weight on length. Both $p(u)$ and $l(u)$ were normalized to fall into the interval $[0,1]$. The empirical cost function thus prefers short and frequent utterances (e.g., \emph{blue}) over long and infrequent ones (\emph{turquoise-ish bananaesque thing}).
...
\ndg{say something here about why the model couldn't fit the higher color-only production in the overinformative (no cc) condition? may be connected to semantic model??}
...
The procedure was identical to that of Exp.~1.\footnote{Exp.~3 constitutes a replication of the production experiment reported in \citeA{GrafEtAl2016}. \ndg{we need to be a little bit careful about how we phrase this (and similar above) so that reviewers don't think this isn't new with this paper. perhaps say "a separate earlier data set was reported at the annual meeting of the cognitive science society, and serves as a close replication if the reported study"?}} Participants proceeded through 72 trials. Of these, half were critical trials of interest and half were filler trials (the critical trials from Exp.~1). On critical trials, we varied the level of reference that was sufficient to mention for uniquely establishing reference.
...
\ndg{need to say somewhere that we ran a typicality elicitation as in previous section for these items...}
...
We evaluated the continuous semantics RSA model on the production data from Exp.~3. The architecture of the model is identical to that of the model presented in \sectionref{sec:colorypicalitymodel}. The only difference is that the set of alternatives contained only the three potential target utterances (i.e., the target's sub, basic, and super label).\footnote{See \tableref{tab:modeldiffs} for an overview of the models reported in the paper.} Whereas the modifier models from the previous sections treat all individual features and feature combinations represented in the display as utterance alternatives, the nominal choice model considers only the three different levels of reference to the target as alternatives, e.g., \emph{dalmatian}, \emph{dog}, \emph{animal}. (So, when a German Shepherd is a distractor, \emph{German Shepherd} is \emph{not} considered an alternative. This has minimal effects on model predictions as long as \emph{German Sheperd} has low semantic fit to the Dalmatian target.) \ndg{why didn't we include the terms for distractors in the alternatives here?}
...
The correlation between empirical utterance proportions and the model's MAP predictions at the by-item level was $r = .86$.
\ndg{also report correlation at the collapsed utterance class level?}
...
\midrule
Semantic values applied to \dots & modifier type (size, color) & utterance type (color, type) and utterance-object combination (e.g., \emph{yellow banana} as referring to various bananas) & utterance-object combination (e.g., \emph{dalmatian} as referring to various dogs)\\
\midrule
Semantic values were\dots & inferred & inferred and elicited experimentally  & elicited experimentally\\
\midrule
Size of lexicon $\mathcal{L}(u,o)$  & 8 (all combinations of \emph{size}, \emph{color}, \emph{othersize}, and \emph{othercolor}) & 814 (1 for each $u,o$ combination) & 51 (1 for each target $u,o$ combination)\\
\midrule
Contextual set of alternatives & all contextually available feature combinations (size, color) & all contextually available feature combinations (type, color) & 3 target alternatives (level of reference: \emph{sub, basic, super}) \\
\midrule
...
\midrule
Cost function & $\beta_{c(\textrm{color})}\mathbbm{1}_{color}(u)+\beta_{c(\textrm{size})}\mathbbm{1}_{size}(u)$ & comparison between $\beta_Fp(u) + \beta_Ll(u)$ vs.~ $\beta_{c(\textrm{color})}\mathbbm{1}_{color}(u)+\beta_{c(\textrm{type})}\mathbbm{1}_{size}(u)$ vs.~none & comparison between $\beta_Fp(u) + \beta_Ll(u)$ vs.~none \\
\midrule
...
%\midrule
Free parameters inferred in BDA & $x_{\textrm{color}}$, $x_{\textrm{size}}$, $\beta_{c(\textrm{color})}$,  $\beta_{c(\textrm{size})}$, $\beta_i$ & $\beta_F$, $\beta_L$, $\beta_i$, $\beta_t$ & $\beta_F$, $\beta_L$, $\beta_i$, $\beta_t$ \\
\bottomrule
\end{tabular}
