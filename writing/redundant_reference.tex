...
While the core architecture with relaxed semantics remained constant throughout the paper, some peripheral components were adjusted to accommodate the aims of different experiments.
...
Most prominently, because Exps.~2 and 3 aimed to predict patterns of reference via typicality at the \emph{object-level}, we needed semantic values for each utterance-object pair in the lexicon.
...
\makecell{Free\\ parameters} & 
	...
	\makecell{$x_{\textrm{color}}$, $x_{\textrm{type}}$, \\ $\beta_i$, $\beta_t$, $w_{\textrm{emp}}$} & 
