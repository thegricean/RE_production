...
The procedure was identical to that of Exp.~1.\footnote{Exp.~3 constitutes a replication of \citeA{GrafEtAl2016}.} Participants proceeded through 72 trials. Of these, half were critical trials of interest and half were filler trials (the critical trials from Exp.~1). On critical trials, we varied the level of reference that was sufficient to mention for uniquely establishing reference.
...
\caption{Utterance probabilities across different conditions. Columns indicate utterances, rows indicate data type (empirical proportion, MAP estimates of posterior predictives for full model with cost and continuous semantics.\jd{recreate these plots with (reduced) condition as facets, diffferent utts in colors, x-axis sub-to-basic typicality, to make it more comparable to exp 2? and then just report the length/frequency effects without plotting them}}
