...
In the full \emph{fixed plus empirical semantics} version, we introduced a parameter $\beta_{fixed}$ interpolating between the empirically elicited typicality values ($\beta_{fixed}=0$) and the inferred type-level values as employed in \sectionref{sec:rsaevaluationbasicscene} ($\beta_{fixed}=1$).
...
In a lesioned \emph{empirical semantics} version, we set $\beta_{fixed}=0$ and only used the empirical values.
Conversly, in a lesioned \emph{fixed semantics} version, we set $\beta_{fixed}=1$ and only used the inferred type-level values. 
...
First, we observe that the semantic interpolation value was highly skewed toward 0, strongly indicating that empirical typicality values strongly improve model performance over a deterministic baseline. \todo[inline]{TODO: Add this facet to Fig. 19; also can we manually fix the facet titles to be the latex? (it's slightly confusing to have `beta\_typ' compared to $\beta_t$ in the caption/text.) }
