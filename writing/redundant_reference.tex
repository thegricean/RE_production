...
In Sections 3, 4, and 5 we quantitatively evaluate cs-RSA on new datasets capturing the phenomena described in the Introduction (\tableref{tab:effects}): modifier type and scene variation effects on modified referring expressions, typicality effects on color mention, and the choice of taxonomic level of reference in nominal choice.
...
While the PRO model -- the most state-of-the-art computational model of human production of modified referring expressions -- can capture the color-size asymmetry discussed above, it does not straightforwardly  account for the more subtle systematicity with which the preference to overmodify with color changes based on scene variation or object typicality, which we turn to next. 
...
\section[]{Experiment 2: color typicality effects}
