...
\begin{figure}
	...
	\caption{Mean typicality ratings for the three norming studies (type-only, color-only, color-and-type). The results are categorized according to the objects' a priori typicality. A yellow banana was designed to be a typical representation and therefore this is its a priori typicality. The brown and blue bananas are midtypical and atypical objects respectively. The category \emph{other} comprises all utterance-object combinations, where the utterance is considered inadequate for the object (for example the utterance "apple" for the object banana). This is analogous to \tableref{tab:bananatypicalities}. Error bars indicate bootstrapped 95\% confidence intervals.}
