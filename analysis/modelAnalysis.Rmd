...
params.samples$parameter2 = factor(params.samples$parameter,
                                   labels = c("informativity","frequency","length","typicality", "semanticInterp"),
...
nomParamPosteriors = ggplot(params.samples, aes(x=value, alpha = .2))+
    geom_histogram(aes(y=..density..),
                   data=subset(params.samples, parameter2 == "informativity"),
                   binwidth = .1, colour="black", fill="white") +
    geom_histogram(aes(y=..density..),
                 data =subset(params.samples, parameter2 == "length" ),
                 binwidth = .1, colour="black", fill="white") +
      geom_histogram(aes(y=..density..),
                   data=subset(params.samples, parameter2 == "frequency"),
                   binwidth = .1, colour="black", fill="white") +
    geom_histogram(aes(y=..density..),
                 data =subset(params.samples, parameter2 == "typicality" ),
                 binwidth = .1, colour="black", fill="white") +
    geom_histogram(aes(y=..density..),
...
Load predictives and get into right format.
```{r}
pred = read.csv("../models/bdaOutput/nominal_cost-empirical_sem-truthconditionalplusempirical_predictives2.csv",quote="")
