...
Read parameter data from BDA and compute MAPs and HDIs.
```{r}
costs = "fixed" # or "empirical"
costs = "empirical" # or "fixed"
semantics = "empirical" # or "fixed" or "fixedplusempirical"
params <- read_csv(paste("../models/bdaOutput/typicality_",costs,"_",semantics,"_params.csv",sep="")) %>%
...
typParamPosteriors = ggplot(params.samples, aes(x=value))+
  ...
  ylab("Density") 
typParamPosteriors
ggsave(file=paste("../writing/pics/exp2-",costs,"-",semantics,"-paramposteriors.pdf",sep=""),width=10,height=3)
```

```{r}
joint = params.samples %>%
  filter(parameter %in% c("colorCost","typeCost")) %>%
  mutate(Iteration=rep(seq(1,numSamples),2)) %>%
...
```{r}
emp = read.csv("../models/bdaInput/typicality/bda_data.csv")
colnames(emp) = c("Condition","TargetColor","TargetType","Dist1Color","Dist1Type","Dist2Color","Dist2Type","Utterance")
...
Load predictives and get into right format.
```{r}
pred = read.csv("../models/bdaOutput/typicality_predictives.csv",quote="")
colnames(pred) = c("Condition","TargetColor","TargetType","Dist1Color","Dist1Type","Dist2Color","Dist2Type","Utterance","logProbability","Zero")
predictives = pred %>%
  mutate(Probability = exp(logProbability),
         Target=paste(TargetColor,"_",TargetType,sep="")) %>%
  # filter(as.character(Utterance) %in% c(as.character(TargetColor),as.character(TargetType),Target)) %>%
  mutate_at(vars(Utterance,TargetColor,TargetType),funs(as.character(.))) %>%
  mutate(UtteranceType = case_when(
    Utterance == TargetColor ~ "color",
    Utterance == TargetType ~ "type",
    Utterance == Target ~ "color-type",
    TRUE ~ "other"
    )) %>%
  # filter(Utterance == TargetColor | Utterance == TargetType | Utterance == Target) %>%
  select(Condition,Target,Utterance,UtteranceType,Probability) %>%
  group_by(Condition,Target,UtteranceType) %>%
...
predictives$TypeTypicality = typ[as.character(predictives$Target),]$Typicality
# predictives$UtteranceType = ifelse(as.character(predictives$Utterance) %in% as.character(unique(pred$TargetColor)),"color",ifelse(as.character(predictives$Utterance) %in% as.character(unique(pred$TargetType)),"type",ifelse(as.character(predictives$Utterance) %in% as.character(unique(predictives$Target)),"color-type","other")))
```

Plot model predictions
```{r}
ggplot(predictives, aes(x=TypeTypicality,y=ModelProb,color=UtteranceType)) +
  # geom_errorbar(aes(ymin=YMin,ymax=YMax)) +
...
ggsave(file="../writing/pics/exp2_predictives.pdf",height=5,width=8)
```
...
answererParamPosteriors = ggplot(params.samples, aes(x=value))+
  #geom_histogram(aes(y=..density..)) +
    ...
    geom_density(aes(y=..density..),
                 data=subset(params.samples, parameter == "typWeight"),
                 adjust = 5, infWeight=.2, fill="#FF6666")+
    geom_density(aes(y=..density..),
                 data=subset(params.samples, parameter == "cost_color"),
                 adjust = 5, infWeight=.2, fill="#FF6666")+
    geom_density(aes(y=..density..),
