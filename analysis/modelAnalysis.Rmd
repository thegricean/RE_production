...
nomParamPosteriors = ggplot(params.samples, aes(x=value))+
  ...
  ylab("Density") 
nomParamPosteriors
ggsave(file="../writing/pics/exp3-paramposteriors.pdf",width=10,height=3)
```


JD: CONTINUE HERE LOADING THE RIGHT EMPIRICAL AND PREDICTIVES DATA FOR EXP 2 AND 3 (AND MERGE IN TYPICALITY VALUES)
...
colnames(emp) = c("Condition","TargetColor","TargetItem","Dist1Color","Dist1Item","Dist2Color","Dist2Item","Utterance")
...
colnames(pred)=c("Condition","TargetColor","TargetItem","Dist1Color","Dist1Item","Dist2Color","Dist2Item","Utterance","logProbability","Zero")
predictives = pred %>%
  mutate(Probability = exp(logProbability)) %>%
  select(Condition,TargetColor,TargetItem,Dist1Color,Dist1Item,Dist2Color,Dist2Item,Utterance,Probability) %>%
  mutate(Data = "model",
         YMin = NA,
         YMax = NA)
```
