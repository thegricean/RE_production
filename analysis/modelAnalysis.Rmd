...
answererParamPosteriors = ggplot(params.samples %>% filter(parameter %in% c('colorTyp', 'sizeTyp')), aes(x=value))+
    ...
    geom_density(aes(y=..density..),
                 data=subset(params.samples, parameter == "colorTyp"),
                 adjust = 2.5, alpha=.2, fill="#FF6666")+
    geom_density(aes(y=..density..),
                 data=subset(params.samples, parameter == "sizeTyp"),
                 adjust = 2.5, alpha=.2, fill="#FF6666")+
...
Clearly see that color is much greater than size (basically non-overlapping posteriors)
...
```{r}
answererParamPosteriors = ggplot(params.samples %>% filter(parameter %in% c('colorCost', 'sizeCost')), aes(x=value))+
    geom_histogram(aes(y=..density..),
