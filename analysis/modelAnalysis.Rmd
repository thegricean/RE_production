...
tiff(file="../writing/pics/fig6.tiff",units="in",width=5.5,height=2.7,res=600)
grid.arrange(typParamPosteriors,costParamPosteriors,nrow=1)
dev.off()
```
...
Make Fig 6 plot analogous to Fig 4.
```{r} 
scale_value=1
...
ggsave("../writing/pics/fig6.tiff",units="in",width=2.7,height=1.7,dpi=600)
```

Make Fig 7 a scatterplot of all data points fit in Exp 1.
```{r} 
...
ggsave("../writing/pics/fig7.tiff",units="in",width=2.7,height=2.8,dpi=600)
```
...
ggsave("../writing/pics/fig14.tiff",units="in",width=5.7,height=1.3,dpi=600)
```
...
tiff(file="../writing/pics/fig13.tiff",units="in",width=3.6,height=6.5,res=600)
...
nomParamPosteriors = ggplot(params.samples, aes(x=value, alpha = .2))+
   ...
   #   geom_density(aes(y=..density..),
    ...
    xlab("Value") +
    theme(panel.border = element_rect(size=.2),
        plot.margin = unit(x = c(0.03, 0.06, 0.03, 0.03), units = "in"),
        panel.grid = element_line(size = .4),
        axis.line        = element_line(colour = "black", size = .2),
        axis.ticks       = element_line(colour = "black", size = .2),
        axis.ticks.length = unit(2, "pt"),
...
ggsave("../writing/pics/fig17.tiff",units="in",width=5.7,height=1.3,dpi=600)
```
...
ggplot(toplot, aes(x=typ_sub,y=Probability,color=relevel(as.factor(Utterance),"sub"))) +
  ...
  geom_text(aes(label=item),nudge_y=.08,size=2) +
  ...
  guides(color=guide_legend(title="Utterance",override.aes=list(fill=NA))) +
  theme(panel.border = element_rect(size=.2),
        plot.margin = unit(x = c(0.03, 0.06, 0.1, 0.03), units = "in"),
        panel.grid = element_line(size = .4),
        axis.line        = element_line(colour = "black", size = .2),
        axis.ticks       = element_line(colour = "black", size = .2),
        axis.ticks.length = unit(2, "pt"),
        ...
        legend.box.margin=margin(0,0,0,0), # change space between legend and plot
        legend.key=element_blank(),
        strip.text      = element_text(size = 6 * scale_value,margin=margin(t=3,b=3,l=3,r=3,unit="pt")))
        # plot.tag.position = c(0,1)
# ggsave("../writing/pics/results-exp3.png",height=4,width=7.5)
ggsave("../writing/pics/fig16.tiff",units="in",width=5.3,height=3.2,dpi=600)
ggsave("../writing/pics/fig16.eps",units="in",width=5.3,height=3.2)#,device=cairo_ps,fallback_resolution = 600)#,dpi=600)
```
